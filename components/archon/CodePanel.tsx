...
export function CodePanel({ code, language = "solidity", footer = "Solidity 0.8.24", height = 260 }: { code: string; language?: string; footer?: string; height?: number }) {
  ...
  const editorTheme = archonMonacoTheme(theme);
  return <section className={fullscreen ? "fixed inset-4 z-50 overflow-hidden rounded-card border border-green-400/30 bg-code-surface shadow-lift" : "overflow-hidden rounded-card border border-border-subtle bg-code-surface shadow-card"}>
    ...
    <Editor height={fullscreen ? "calc(100vh - 112px)" : height} defaultLanguage={language} value={code} theme={editorTheme} beforeMount={defineArchonMonacoThemes} options={{ readOnly: true, minimap: { enabled: false }, lineNumbers: "on", fontFamily: "var(--font-mono)", fontSize: 13, automaticLayout: true }} />
