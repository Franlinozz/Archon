...
// Inline banner for a transient backend (DB/queue) failure. Lets a page render at HTTP 200
// with empty data instead of throwing into a 500. The connection self-heals on the next request.
export function DegradedNotice({ resource = "Live data" }: { resource?: string }) { return <div className="flex items-center gap-3 rounded-card border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning"><AlertTriangle size={16} className="shrink-0"/><span>{resource} is temporarily unavailable — values show empty until the connection recovers. No scan or on-chain action was affected.</span></div>; }
