...
export default async function PublicReportPage({ params }: { params: Promise<{ reportId: string }> }) {
  ...
  let attestation: { id: string; matchType: string } | null = null;
  let degraded = false;
  try {
    ...
    if (report) {
      // Verified-build marker: a passing attestation for the scanned address
      // proves the audited source corresponds to the deployed bytecode (F2).
      const attRes = await db.query<{ id: string; matchType: string }>(
        `select a.id, a.match_type as "matchType" from attestations a join scans s on s.id=$1 and s.source_kind='address' and lower(s.source_ref)=a.address
          where a.status='done' and a.match_type in ('exact','partial-metadata') order by a.created_at desc limit 1`,
        [report.scanId],
      ).catch(() => ({ rows: [] as Array<{ id: string; matchType: string }> }));
      attestation = attRes.rows[0] ?? null;
    }
    if (report) {
      const [findingsResult, proofResult, challengesResult] = await Promise.all([
        ...
        db.query<Challenge>(`select id, target_type as "targetType", challenger, title, rationale, evidence_url as "evidenceUrl", status, challenge_hash as "challengeHash", reference_tx_hash as "referenceTxHash", reference_report_hash as "referenceReportHash", created_at as "createdAt" from report_challenges where report_id=$1 order by created_at desc`, [reportId]),
      ]);
