...
export default async function Page() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-4xl font-bold tracking-tight text-text-hi">On-chain Proof & Reports</h1><p className="mt-2 max-w-3xl text-text-mid">Verify Archon audit reports against deterministic report hashes, stored metadata, and on-chain proof records — anchored to ArchonProofRegistry with an ERC-8004 Reputation entry against Agent #97.</p></div><span className="rounded-pill border border-success/30 bg-success/10 px-3 py-1 text-sm text-success">Mantle proof path · ArchonProofRegistry + ERC-8004 Reputation</span></div>{degraded ? <DegradedNotice resource="On-chain proof records"/> : null}{!validation.available ?<p className="rounded-card border border-border-subtle bg-surface-1 p-3 text-sm text-text-mid">{validation.note} Validation/challenge flows are hidden until an official address is published.</p> : null}
    <div className="grid gap-6 [&>*]:min-w-0 xl:grid-cols-[1fr_380px]">
      <section className="rounded-card border border-border-subtle bg-surface-1 p-5">
        <div className="flex flex-wrap items-center gap-2"><span className="rounded-pill border border-green-400/35 bg-green-400/10 px-3 py-1.5 text-sm text-green-400">Report History</span><span className="rounded-pill border border-border-subtle bg-surface-2 px-3 py-1.5 text-sm text-text-mid">Proof Verification</span><span className="ml-auto text-xs text-text-low">{rows.length} {rows.length === 1 ? "proof" : "proofs"}</span></div>
        ...
        <section className="mt-5 rounded-card border border-border-subtle bg-terminal p-4"><p className="text-xs uppercase tracking-[0.14em] text-green-400">Proof Verification</p><p className="mt-2 text-sm text-text-mid">The verification tab re-derives the report hash from stored canonical metadata and compares it with the proof row. A logged proof anchors to ArchonProofRegistry (permissionless, idempotent) and appends an ERC-8004 Reputation Registry entry against Agent #97 (feedbackHash = the report hash); the Validation Registry UI is intentionally absent until the official ERC-8004 repo publishes a Mantle Mainnet address.</p>{selected ? <div className="mt-3 grid gap-2 text-sm"><Line label="Stored hash" value={selected.reportHash}/><Line label="Re-derived hash" value={rederived ?? "No metadata"}/><Line label="Result" value={rederived === selected.reportHash ? "Hash match" : "Hash mismatch"}/></div> : null}</section>
      </section>
