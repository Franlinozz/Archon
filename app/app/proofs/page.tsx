  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
      <section className="rounded-card border border-border-subtle bg-surface-1 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex gap-2"><button className="rounded-pill border border-green-400/35 bg-green-400/10 px-3 py-1.5 text-sm text-green-400">Report History</button><button className="rounded-pill border border-border-subtle bg-surface-2 px-3 py-1.5 text-sm text-text-mid">Proof Verification</button></div><input placeholder="Search contract, hash, tx hash…" className="min-w-72 rounded-control border-border-subtle bg-terminal text-sm text-text-hi placeholder:text-text-low" /></div>
...
function CopyButton({ value }: { value: string }) { return <button className="ml-1 inline rounded border border-border-subtle p-1 text-text-low hover:text-green-400" title="Copy" data-copy={value}><Copy size={12}/></button>; }
