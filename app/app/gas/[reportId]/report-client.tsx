...
export function GasReportClient({ report: initialReport, optimizations: initialOptimizations, challenges }: { report: GasReport; optimizations: GasOpt[]; challenges: Challenge[] }) {
  ...
  async function applyPatch(opt: GasOpt) {
    ...
    if (res.status === 202) { setNotice("Patch generation queued in the worker. Refresh this report when it completes."); return; }
    if (!res.ok) { setNotice(data.error ?? "Patch generation failed."); return; }
    await refresh(); setNotice("Patch ready. Cached patched source and Foundry gas proof are attached to this card.");
  }
  ...
  async function applyAllSafe() {
    ...
    if (pending.length) {
      ...
      setNotice(`${pending.length} safe patch${pending.length === 1 ? "" : "es"} queued. Retry Apply-all-safe after worker completion to download the optimized contract.`); return;
    }
