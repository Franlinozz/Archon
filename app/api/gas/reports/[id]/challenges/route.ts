...
import { z } from "zod";
import { challengeInputSchema, createGasChallenge, listGasChallenges } from "@/lib/challenges/service";
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ schema: "archon.challenges.v1", challenges: await listGasChallenges(params.data.id) });
}
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const challenge = await createGasChallenge(params.data.id, parsed.data);
