...
const mnt = (wei: bigint | number | string) => Number(typeof wei === "bigint" ? wei : BigInt(String(wei).split(".")[0] || "0")) / 1e18;
...
export async function getObservatory() {
  const [model, current, windows, oracleSeries, oracleAgg, recentSamples] = await Promise.all([
    ...
    db.query<{ window: string; da_per_byte: string | null; n: number }>(
      `select w.window, percentile_cont(0.5) within group (order by s.l1_fee_wei / nullif(s.total_bytes,0)) as da_per_byte, count(s.*)::int as n
         from (values ('24h', interval '24 hours'), ('7d', interval '7 days'), ('30d', interval '30 days')) as w(window, span)
  ...
  const daPerByteWei = cur?.da_per_byte ? BigInt(cur.da_per_byte.split(".")[0] || "0") : null;
  const l2BaseFeeWei = cur?.l2_base_fee ? BigInt(cur.l2_base_fee.split(".")[0] || "0") : null;
  ...
  const liveActual = oracleAgg.rows[0]?.actual_sum ? BigInt(oracleAgg.rows[0]!.actual_sum!.split(".")[0] || "0") : 0n;
  const liveOracle = oracleAgg.rows[0]?.oracle_sum ? BigInt(oracleAgg.rows[0]!.oracle_sum!.split(".")[0] || "0") : 0n;
  ...
  return {
    ...
    trends: windows.rows.map((w) => ({ window: w.window, daPerByteGwei: w.da_per_byte ? Number(BigInt(w.da_per_byte.split(".")[0] || "0")) / 1e9 : null, sampleCount: w.n })),
    cards,
    oracle: {
      live: liveDivergence,
      ...
      series: oracleSeries.rows.filter((r) => r.oracle).map((r) => ({ bucket: r.bucket, actualGwei: r.actual ? Number(BigInt(r.actual.split(".")[0] || "0")) / 1e9 : null, oracleGwei: r.oracle ? Number(BigInt(r.oracle.split(".")[0] || "0")) / 1e9 : null, n: r.n })),
    },
