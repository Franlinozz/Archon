...
The app (Audit Studio, Gas Optimizer, Cost Guard, proofs, validation, [Sentinel](/docs/audit/sentinel) continuous monitoring, and [verified builds](/docs/on-chain-proofs/verified-builds)), public surfaces (landing, [docs](/docs), [public reports](/proofs), [leaderboard](/gas-leaderboard), the [Gas Observatory](/observatory), and a permanent per-contract [address page + badge](/docs/platform-api/public-pages-badges)), the REST API with [OpenAPI 3.1 reference](/api-reference) — including a signed [agent verdict + MCP server](/docs/platform-api/for-agents) — the [`archon-scan` CLI](/docs/platform-api/cli), the [CI GitHub Action](/docs/gas-optimizer/ci-github-action) and [GitHub App](/docs/platform-api/github-app) with live PR evidence, the [VS Code extension](/docs/platform-api/editor-integration) (published on Open VSX), planned webhooks, and report/test/patch exports. AI and storage run on the [cloud provider layer](/docs/platform-api/cloud-providers): **AI reasoning is served on Tencent Cloud TokenHub** (`deepseek-v4-pro`) with **OpenAI `gpt-4o-mini` as runtime failover**, and **Tencent Cloud COS is a live, best-effort artifact backup** (region `ap-hongkong`) — all status verifiable at [/api/providers](/api/providers). TokenHub serves third-party reasoning models; Archon does not run a Hunyuan model.
