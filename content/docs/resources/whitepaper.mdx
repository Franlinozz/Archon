---
title: Whitepaper
description: Archon whitepaper v2.0 — verifiable DevTools for Mantle. HTML edition; the PDF is the citable edition.
---

# Archon whitepaper

<div className="not-prose my-6 rounded-card border border-brand-500/25 bg-surface-2 p-5 shadow-card">
  <p className="m-0 text-xs font-semibold uppercase tracking-[0.18em] text-brand-700">Archon Whitepaper · V2.0 · June 2026</p>
  ...
  <div className="mt-4 flex flex-wrap gap-2">
    <a href="/whitepaper.pdf" className="inline-flex rounded-input bg-green-500 px-4 py-2 text-sm font-bold text-on-green shadow-sm hover:bg-green-400">Download PDF (v2.0)</a>
...
The app (Audit Studio, Gas Optimizer, Cost Guard, proofs, validation), public surfaces (landing, [docs](/docs), [public reports](/proofs), [leaderboard](/gas-leaderboard)), the REST API with [OpenAPI 3.1 reference](/api-reference), the [`archon-scan` CLI](/docs/platform-api/cli), the [CI GitHub Action](/docs/gas-optimizer/ci-github-action) with live PR evidence, webhooks, and report/test/patch exports. AI and storage run on the [cloud provider layer](/docs/platform-api/cloud-providers) (OpenAI live; ELFA and Tencent Cloud Hunyuan/COS adapters built-in, inert until credentialed — status at [/api/providers](/api/providers)).
...
| Horizon | Direction |
| --- | --- |
| **Now (live)** | Audit and gas engines on Mantle Mainnet; proof anchoring in both modes; public verification, leaderboard, challenge ledger; docs, CLI, CI Action, generated tests. |
| **Next** | Reliability hardening across upload/repo inputs; public API keys and webhooks; deeper protocol packs; continuous monitoring of deployed contracts with drift alerts; cloud provider adapters (including Tencent Cloud) activated on provisioned credits; pricing made public. |
| **Later** | ERC-8004 Validation Registry integration when an official Mantle address ships; verified-build attestations; cross-agent attestations; staked challenges; agent-consumable trust APIs; team workspaces and ecosystem dashboards. |
...
- **Source:** [github.com/Franlinozz/Archon](https://github.com/Franlinozz/Archon) · **Citable edition:** [whitepaper PDF v2.0](/whitepaper.pdf)
